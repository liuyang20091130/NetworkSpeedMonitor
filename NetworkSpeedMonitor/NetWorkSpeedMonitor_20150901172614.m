...
- (NSString *)speedStr {
    NSString  *speedStr = [self bytesToSpeedStr:_bytesPerSecond];
    return speedStr;
}
...
- (void)reSetBtyesPersceond:(NSDictionary *)bytesDic {
    _networkBytesPerSecond.inBytes = [[bytesDic objectForKey:@"inBytes"] longLongValue];
    _networkBytesPerSecond.outBytes = [[bytesDic objectForKey:@"outBytes"] longLongValue];
